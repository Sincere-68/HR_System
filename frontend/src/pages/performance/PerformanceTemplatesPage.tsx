...
import { Alert, Button, Empty, Skeleton, Table, Tag, type TableColumnsType } from 'antd';
import { useNavigate } from 'react-router-dom';
import { usePerformanceTemplates } from '../../features/performance/api';
...
export function PerformanceTemplatesPage() {
  ...
  const columns: TableColumnsType<PerformanceTemplateListItem> = [
    ...
    { title: '操作', key: 'actions', width: 130, render: (_, record) => <Button type="link" onClick={() => navigate(`/performance/templates/${record.id}`)}>配置模板</Button> },
  ];

  ...
  return <main className="performance-feishu-inbox-page">
    ...
    {inbox.isLoading ? <Skeleton active paragraph={{ rows: 8 }} /> : <>
      ...
      <Card title="待提交评价" className="performance-feishu-inbox-card">
        {assessmentRows.length === 0 ? <Empty description="暂无待提交评价" /> : <List<PerformanceTaskListItem> dataSource={assessmentRows} renderItem={(task) => <List.Item actions={[<Button key="handle" type="link" onClick={() => setAssessmentTarget(task)}>处理</Button>]}><List.Item.Meta title={task.moduleName} description={<Space direction="vertical" size={0}><span>{task.employeeName}（{task.employeeNo}）</span><span>下发指标：{task.indicators.length ? task.indicators.map((indicator) => indicator.name).join('、') : '无'}</span></Space>} /><Tag>{task.moduleType === 'EVALUATION' ? '人工评估' : '结果调整'}</Tag></List.Item>} />}
      </Card>
    ...
    <Modal title={assessmentTarget ? `提交：${assessmentTarget.moduleName}` : ''} open={Boolean(assessmentTarget)} onCancel={() => setAssessmentTarget(null)} footer={null} destroyOnHidden>
      {assessmentTarget ? <Form form={assessmentForm} layout="vertical" onFinish={submitAssessmentValue}><Typography.Paragraph type="secondary">下发指标：{assessmentTarget.indicators.length ? assessmentTarget.indicators.map((indicator) => `${indicator.name}（${indicator.standards.length ? indicator.standards.join('；') : '暂无衡量标准'}）`).join('；') : '无'}</Typography.Paragraph><Form.Item name={assessmentTarget.moduleType === 'EVALUATION' ? 'score' : 'adjustment'} label={assessmentTarget.moduleType === 'EVALUATION' ? '评分（0-100）' : '调整分值'} rules={[{ required: true, message: '请输入分值' }]}><InputNumber min={assessmentTarget.moduleType === 'EVALUATION' ? 0 : 0} max={assessmentTarget.moduleType === 'EVALUATION' ? 100 : undefined} style={{ width: '100%' }} /></Form.Item><Form.Item name="comment" label="评语"><Input.TextArea rows={4} /></Form.Item><Button type="primary" htmlType="submit" loading={submitAssessment.isPending}>提交</Button></Form> : null}
    </Modal>

...
请使用下载模板中的中文业务表头（例如“工号、姓名、部门”）。除完整确认的旧“全部在职”导出列外，系统不猜测外部字段含义；标准字段严格使用“籍贯、户口类别、户籍所在地、联系地址”，并按 Excel 原文保存。旧拆分字段已删除；旧导出中的 parent_Birthplace、parent_RegistAddress、parent_HomeAddress 分别兼容导入籍贯、户籍所在地、联系地址。工号已存在时只更新文件中有值的字段，未填写的字段保持原值。导入不使用页面新增人员或补建任职的必填规则：新工号始终创建主档；当来源能匹配部门并提供用工形式时，会保存任职周期和部门任职，入职日期、雇佣关系及人员状态等缺失字段保留为空或采用已有安全默认值；不满足数据库固有字段要求的任职或独立关系会在结果中提示，但不会阻止同一行其他字段保存。工作地点按输入文本直接保存；职位仅填写确认的职位名称，未匹配只提示；全日制公司和直线经理仅在能唯一匹配已有目录/员工且员工已有任职周期时关联。证件、紧急联系人和教育经历均按文件中已提供的字段保存，其他字段可以为空；无有效经理、部门等关联对象时只提示该关联字段。累计工龄（年）按导入基数保存，之后新增的工作履历按日期继续累加。

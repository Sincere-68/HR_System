...
请使用下载模板中的中文业务表头（例如“工号、姓名、部门”）。系统不猜测外部系统英文/编码字段含义；工号已存在时仅更新文件中有值的字段。工作地点按输入文本直接保存；职位仅填写确认的职位名称，系统会关联同名的有效职位目录；职位编号及“编号 - 名称”格式已不再使用，未匹配会在结果表提示。教育经历首次创建必须同时提供学校、院校类型、学历、毕业时间和专业；新工号若同一行提供完整的“部门、入职日期、雇佣关系、用工形式、人员状态”，将直接建立首段任职，否则仅创建待完善的人员主档。

...
export const performanceApi = {
  ...
  createCycleParticipantAmountBase: ({ cycleId, employeeId, input }: { cycleId: string; employeeId: string; input: PerformanceCreateEmployeePerformanceAmountBaseInput }) => apiRequest(`/performance/cycles/${cycleId}/participants/${employeeId}/amount-base`, { method: 'POST', body: JSON.stringify(input) }),
  ...
  removeCycleParticipant: ({ cycleId, employeeId }: { cycleId: string; employeeId: string }) => apiRequest<PerformanceCycleDetail>(`/performance/cycles/${cycleId}/participants/${employeeId}`, { method: 'DELETE' }),
...
export function useCreateCycleParticipantAmountBase() { const client = useQueryClient(); return useMutation({ mutationFn: performanceApi.createCycleParticipantAmountBase, onSuccess: (_, variables) => { client.invalidateQueries({ queryKey: performanceKeys.cycle(variables.cycleId) }); } }); }
...
export function useRemovePerformanceCycleParticipant() { const client = useQueryClient(); return useMutation({ mutationFn: performanceApi.removeCycleParticipant, onSuccess: (_, variables) => { client.invalidateQueries({ queryKey: performanceKeys.all }); client.invalidateQueries({ queryKey: performanceKeys.cycle(variables.cycleId) }); } }); }

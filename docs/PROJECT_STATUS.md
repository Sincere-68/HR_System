...
| 组织树 | `Organization` | 通过 `parentId` 组成可扩展树；组织范围包含选中组织及其所有下级。 |
...
| 页面 | 前端路由 | 后端接口 | 当前状态 |
|---|---|---|---|
| Offer 管理 | `/onboarding/offers` | `GET /onboarding/offers` | 已实现待发、已发、已接受、已拒绝、已入职、全部六张可点击状态卡及对应只读表格；卡片数量和列表均受后端组织范围限制。 |
...
| 职位、职级、工作地点 | `positionName`、`jobLevel`、`workplaceName` | 当前主要任职关联的 `Position`、固定 `JobLevelCode` code、`Workplace`；职级允许 `S1`–`S7`、`E1`–`E7`、`T1`–`T7`、`M1`–`M7`。 |
...
| 民族、婚姻状况、政治面貌、籍贯 | `ethnicity`、`maritalStatus`、`politicalStatus`、`nativePlace` | `Employee` 同名业务字段。 |
| 户口类别 | `householdType` | `Employee.householdType`；尚未录入时返回 `null/--`。 |
| 户籍所在地、联系地址 | `householdAddress`、`residentialAddress` | `Employee.householdAddress`、`Employee.residentialAddress`。 |
...
| 操作 | — | 六张表均禁用“暂无操作”；当前没有 Offer 详情或维护 API。 |
...
- 人员 PATCH 已移除旧 `idCardNo` 输入，只接受规范证件字段。居民身份证与其他主要证件间切换、仅改号码及仅改截止日期都会在同一事务中同步兼容字段；完整证件记录缺失时必须一并给出类型和号码。
...
4. 录用入职五页、合同和调动类型目前为只读列表，操作按钮禁用；不要在文档中推断不存在的维护能力。
...
11. 当前工作区包含大量尚未提交的增量功能，继续开发前必须先查看 `git status` 和相关 diff，不得覆盖或回退已有改动。

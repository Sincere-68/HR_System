...
| 民族、婚姻状况、政治面貌 | `ethnicity`、`maritalStatus`、`politicalStatus` | `Employee` 同名业务字段。 |
| 籍贯 | `nativePlaceRegionName` | `Employee.nativePlaceRegionName` 原文；不校验地区目录或编码。 |
| 户口类别 | `householdType` | `Employee.householdType`；尚未录入时返回 `null/--`。 |
| 户籍所在地、联系地址 | `householdRegionName`、`residentialRegionName` | `Employee` 同名字段按来源原文保存。旧 code/详细字段只作数据库历史兼容，不再由人员 API 或页面公开。 |

...
以下绩效接口均为本地 HR 系统能力，不读取、复制或调用任何第三方业务系统数据。飞书仅在已配置并授权的情况下向个人发送卡片；不会向群聊发送消息。

### GET `/performance/cycles/:id`

需要 `performance.read`。返回活动及其被考核人列表。被考核人行按活动实际实例返回员工、部门、模板、指标模板、当前步骤、当前执行人、考核组、状态、总分、总等级、人员状态和最终系数；没有可靠来源的字段为 `null`，前端显示 `--`。后端按活动组织及实例组织强制校验当前账号的数据范围，并写入活动详情查看审计。

### GET `/performance/cycles/:cycleId/participants/:instanceId/workflow`

需要 `performance.read`。返回指定活动内指定被考核人的真实步骤历史；`instanceId` 必须同时属于 `cycleId`，且请求者必须有对应组织范围。响应包含活动/人员摘要与 `steps`：人工评估、调整模块及后续审核、确认、审批、HR 归档步骤均从已生成的任务历史读取，按真实步骤顺序和退回后的 `attemptNo` 返回，不会根据当前模板生成虚假步骤。

每个步骤及执行人返回：执行人、配置角色、步骤状态、通知投递状态、送达时间、提交时间、是否达标及投递失败原因。`CONFIRMATION` 本人确认的执行人固定为对应 `PerformanceInstance.employee`，在活动实例绑定被考核人后创建任务时快照，不从模板中选择或重新推断。送达时间只在飞书个人卡片发送成功后写入；未启用渠道、无可用唯一飞书身份或发送失败时为 `null`，并保留“未送达”或失败原因。提交时间仅取执行人真实完成任务的 `completedAt`；未提交为 `null`。当前没有已确认的统一“是否达标”阈值，因此该字段返回 `null`/`--`，不得根据分数猜测。
...
- 普通员工信息审批卡片仅显示通过/驳回和选填审批意见，不含分数。当前通用审批的正式节点推进、驳回和最终业务写入规则仍待 HR 配置，因此卡片提醒不会自行把 `ApprovalStep` 改为通过/驳回或创建新流转。
- 接收方式支持现有 HTTP 卡片回调，以及 `FEISHU_LONG_CONNECTION_ENABLED=true` 时的飞书 SDK 长连接 `card.action.trigger`。长连接适用于内网部署，无需暴露公网 URL；需要在飞书应用后台启用相应的长连接卡片回调。`FEISHU_ENABLED`、`FEISHU_APP_ID`、`FEISHU_APP_SECRET` 仍是必要的服务器环境变量；不得将凭据写入代码、前端或文档。
- 飞书身份仅通过当前指定处理人绑定员工的企业邮箱（以及已有安全联系方式逻辑）解析，不接受前端或卡片传入 `open_id`。企业邮箱缺失、身份不唯一、发送失败、签名/事件校验失败、非当前处理人和重复提交均返回明确业务错误。成功评分后系统尝试更新原卡片为“已提交”；卡片更新失败不会回滚数据库中已完成的评分。

### 通知投递日志

`performance_assessment_notification_deliveries` 和 `performance_workflow_notification_deliveries` 是独立于 `audit_logs` 的投递历史。每次向一个人工评估/调整任务执行人或后续流程执行人尝试飞书个人卡片时各保存一条记录，包含渠道、状态、送达时间和失败原因。只有渠道确认成功后才标记 `DELIVERED` 并写入 `delivered_at`；任务推进、投递失败或未配置飞书都不会回滚绩效业务数据。
